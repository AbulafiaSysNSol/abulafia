...
$inserimento=mysql_query("insert into defaultsettings values('$id', '$risultati_per_pagina', '$annoprotocollo', '$altro','$fototargetpath', '$splash', '$color2', '$color1',  '$larghezzatabellarisultati', '$protocollomaxfilesize', '$fotomaxfilesize', '$nomeapplicativo', '$email' , '$version', '$host', '$paginaprincipale', '$titolopagina', '$keywords', '$description', '$headercescription', '$ins', '$mod')");
if (!$inserimento) {echo 'Impossibile compiere l\'azione richiesta'; echo mysql_error(); exit();}

}


$update=mysql_query("update defaultsettings set risultatiperpagina='$risultati_per_pagina' , splash='$splash' , primocoloretabellarisultati='$color1' , secondocoloretabellarisultati='$color2', notificains = '$ins' , notificamod = '$mod' where idanagrafica='$id'");
if (!$update) {
?>
