...
<?php
	...
	if( isset($_GET['add']) && $_GET['add'] == "no") {
		...
		<div class="row">
			<div class="col-sm-12">
				...
				Si è verificato un errore, controlla di aver inserito tutti
				 i campi oppure riprova più tardi.
				</div>
			</div>
		</div>
		<?php
	...
	if( isset($_GET['add']) && $_GET['add'] == "duplicato") {
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="alert alert-danger">
				Impossibile aggiungere: esiste già un 
				settore con lo stesso nome!
				</div>
			</div>
		</div>
		<?php
	...
	if( isset($_GET['mod']) && $_GET['mod'] == "ok") {
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="alert alert-success">
				<span class="glyphicon glyphicon-ok"></span> 
				Settore modificato con successo!
				</div>
			</div>
		</div>
		<?php
	...
	if( isset($_GET['mod']) && $_GET['mod'] == "no") {
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="alert alert-success">
				<span class="glyphicon glyphicon-ok"></span> 
				Si è verificato un errore nella modifica della descrizione.
				</div>
			</div>
		</div>
		<?php
	...
	if( isset($_GET['canc']) && $_GET['canc'] == "ok") {
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="alert alert-success">
				<span class="glyphicon glyphicon-ok"></span> 
				Settore eliminato con successo!
				</div>
			</div>
		</div>
		<?php
	...
	if( isset($_GET['canc']) && $_GET['canc'] == "no") {
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="alert alert-success">
				<span class="glyphicon glyphicon-ok"></span> 
				Si è verificato un errore nella cancellazione del settore
				</div>
			</div>
		</div>
	<?php
			...
			?>
				...
				foreach ($settori as $val) { 
					...
					<tr>
						...
						<td><?php echo stripslashes($val['descrizione']); ?></td>
						<td>
							<div class="btn-group btn-group-sm">
								<a class="btn btn-warning" 
									...
									href="login0.php?corpus=magazzino-modifica-settore
										&id=<?php echo $val['id']; ?>">
								...
								<?php if($admin) {
										 ?> <a class="btn btn-danger"
											...
											onClick="return confirm('Vuoi veramente cancellare questo settore?');"
											href="login0.php?corpus=magazzino-elimina-settore&id=<?php echo $val['id']; ?>"><span class="glyphicon glyphicon-trash"></span>Elimina
											</a>
										<?php 
									} 
								?>
