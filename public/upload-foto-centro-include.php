<?php
	$target_path = "foto/"; //assegnazione del percorso per l'upload delle foto per l'anagrafica
	if(isset($_GET['id'])) {
	...
	if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
		$my_log -> publscrivilog( $_SESSION['loginname'], 'AGGIUNTA FOTO ANAGRAFICA '. $id , 'OK' , 'NOME FILE '. $urlfoto , $_SESSION['logfile'], 'anagrafica');
		?>
		...
		$my_log -> publscrivilog( $_SESSION['loginname'], 'AGGIUNTA FOTO ANAGRAFICA '. $id , 'FAILED' , 'NOME FILE '. $urlfoto ,$_SESSION['logfile'], 'anagrafica');
		?>
		<SCRIPT LANGUAGE="Javascript">
		browser= navigator.appName;
		if (browser == "Netscape")
