<?php
	...
	if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) { //se lo spostamento del file va a buon fine
		...
		if($from == 'modifica-protocollo') {
			...
			else window.location="login0.php?corpus=protocollo2&anno=<?php echo $annoprotocollo;?>&idlettera=<?php echo $idlettera;?>&from=urlpdf&upfile=success";
			</SCRIPT>
			<?php
		}
		else {
		...
		if($from == 'modifica-protocollo') {
			...
			browser= navigator.appName;
			if (browser == "Netscape")
			window.location="login0.php?corpus=modifica-protocollo&anno=<?php echo $annoprotocollo;?>&idlettera=<?php echo $idlettera;?>&from=urlpdf&upfile=error"; 
			else window.location="login0.php?corpus=protocollo2&anno=<?php echo $annoprotocollo;?>&idlettera=<?php echo $idlettera;?>&from=urlpdf&upfile=error";
			</SCRIPT>
			<?php
		}
		else {
